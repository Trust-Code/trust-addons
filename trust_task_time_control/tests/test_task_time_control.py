...
class TestTaskTime(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.employee = self.env['res.users'].browse(self.env.user.id).employee_ids[0]
        group_id = self.env['ir.model.data'].get_object('base',  'group_hr_attendance')
        ...
        self.reviewer_user = self.env['res.users'].create({
            'name':'teste2', 'login': 'teste', 'company_id': self.env.user.company_id.id                      
        })
    ...
    def test_time_locating(self):
        ...
        self.assertNotEqual(self.task_one.work_ids.user_id.id, self.env.user.id,
